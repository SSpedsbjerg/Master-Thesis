\section{Development}
	...
	\begin{center}
		\resizebox{\textwidth}{!}{
			...
			\begin{tabular}{|p{2cm}|p{2cm}|p{2cm}|p{3cm}|p{3cm}|}
				...
				FR0 & Individual process in each 'Event Node' & Must have &An 'Event Node' is the main process for making predictions and conclusions. & These processes are essential for generating predictions and conclusions.\\ \hline
				...
				FR5 & Event detection & Must have & Detects relevant events occurring within the system. & Core functionality of the system. \\ \hline
	...
	\begin{itemize}
		...
		A simple model will use Roslyn to make determinations, but as the related work (section \ref{sec:RelatedWork}) determined, \gls{ml} models frequently used to make determinations and supporting that will therefore for that reason most likely bring the highest value. Through this project, the following \gls{ml} models in same order will be implemented. \gls{svm}, easy to implement and is the second most used model in the related work. \gls{rf} due to t be very different from \gls{svm}, allowing to determine the range of models which can be used in \gls{reps}. Neural Networks requires more implementation than some of the other 2, while it is the most used, it also requires more work, and as the implementation of models is not the focus of this project, it can be put in the last spot in the queue for implementation.
	\end{itemize}
	\newpage
	\begin{figure}[!h]
		\begin{center}
	...
	In C\#, the output must be defined as the \texttt{object} type. While the \texttt{dynamic} type is an alternative, it introduces runtime casting, which increases the risk of errors. The \texttt{var} keyword is not suitable, as it resolves types at compile time. By using \texttt{object}, I must explicitly define a set of permitted casting types. This reduces flexibility but significantly lowers the risk of runtime errors.
	...
	%https://www.nuget.org/PACKAGES?q=\&includeComputedFrameworks=true\&prerel=true\&sortby=totalDownloads-desc
	For \gls{json} parsing, I will be using Newtonsoft, a C\# framework for interpreting JSON. This choice is based on its ease of use, extensive documentation, and the fact that it is the most downloaded NuGet package[], which suggests that if any issues arise, solutions are likely to be found on Stack Overflow or similar platforms.
	
	%(TODO: gennemgå flere af noterne og skriv om rabbitMQ og threading begrundelserne her)
	The target of the project is to have and be able to create graphs, a worry then arrives that the implementation is a tree that is pretends to be a graph, i.e. that it is not possible to create circular dependencies. Therefore there is a need to remove dependencies from other nodes, that even though the parent node is not finished processing, the child node will just have to use the old output value. This ensures that a circular dependency does not cause an infinite loop and actually allows for it to be a graph.
	\newpage
	\subsection{Design}
	...
	As the project furthers and new iterations of the project is completed, the class diagram will be updated with the final diagram (Figure: \ref{fig:FinalClassDiagram}):
	\begin{figure}
		\begin{center}
...
protected virtual void UpdateEqulibrium<T>((object, TimeOnly)[] values) {
	...
	A Machine Learning (ML) model is an abstract class and is also regarded as an adaptive model. However, in the current implementation, it does not utilise the inherited functionality. The concept is that machine learning models can also adapt as new data becomes available. The Machine Learning model defines two methods which are not yet implemented.
...
public List<(string, string, Guid, int)> GetTypes<T>() {
	...
	\subsection{Research Question 0 : How should a \gls{reps} be constructed}
		...
		In conclusion, the best way to implement \gls{reps} is with each node being independent, allowing varied implementations. Although this project is implemented as a monolith, there is no reason it could not be implemented as microservices. Independence enables nodes to perform calculations concurrently without waiting for others to finish. There should be a clear separation between sensor nodes and event nodes, as their concerns differ, and multiple event nodes can utilise the same sensor data. Event nodes should have only one output, ensuring they remain focused on their task.
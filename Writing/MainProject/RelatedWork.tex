\section{Related Work}\label{sec:RelatedWork}
	...
	The next search engine used is Forskningsportalen, a Danish platform for academic research that includes publications in both Danish and English. It was chosen to reduce duplication of articles found in other databases. The query used is the same as for the Scopus search, with adjustments to match Forskningsportalen’s syntax:
	...
	\subsection{Outcome}
		...
		Wu et al.\cite{WuPan} use a graph-based structure for time series forecasting, its idea is very similar to this project but takes a different approach, it transforms multivariate time series data into a graph which can be used in a graph neural network, The graph structures nodes derives from variables from multivariate time series data. In relation to this project it shows one approach to the same problem of defining a relation between nodes to predict events. (skal det uddybbes videre iforhold til mit projekt? og jeg formoder at det skal gøres det samme med de andre eksempler). \sout{Ribeiro et al.\cite{Ribeiro} Propose a technique to explain how the model came to a prediction which can help the end user in trusting the prediction. Lv et al. \cite{Lv_Duan} Uses deep learning to make traffic flow prediction, one of the scenarios considered early in for this project. Zhang et al.\cite{Zhang} uses Long short-term memory networks to predict remaining useful life.} These articles fall within the domain targeted by the proposed pattern.
		...
		\sout{
		The articles also inform the approach to user interaction. Ribeiro et al.~\cite{Ribeiro} emphasize user trust through model explain-ability, while Wu et al.~\cite{WuPan} contribute structural insights. Based on these, a publish-subscribe pattern will be used for communication between \gls{reps} and external components.
	}